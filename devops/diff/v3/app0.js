...
/* ── Rebuild all overlay widgets for one side editor ── */
function refreshOverlayWidgets(editor, side) {
  const mon = window._monaco;
  const key = side === 'A' ? '_widgetsA' : '_widgetsB';

  // Remove old widgets
  (merge[key] || []).forEach(w => editor.removeOverlayWidget(w));
  merge[key] = [];
  ...
  conflicts.forEach(ch => {
    // Compute pixel top for this conflict's first line
    let lineStart = 1;
    for (const c of merge.chunks) {
    ...
    domNode.className = 'codelens-overlay-widget' +
      (isCurrent ? ' is-current' : '') +
      (chosen ? ' resolved' : ' unresolved');
    domNode.dataset.conflictIdx = ch.idx;
    ...
    // Position the widget over the correct line using the editor's layout API
    const widget = {
      getId()      { return `lens-${side}-${ch.idx}`; },
      getDomNode() { return domNode; },
      getPosition(){ return null; }, // we position manually below
    };

    editor.addOverlayWidget(widget);

    // Reposition: convert line number → pixels and apply as absolute style
    function positionWidget() {
      const top = editor.getTopForLineNumber(lineStart);
      const scrollTop = editor.getScrollTop();
      const scrollLeft = editor.getScrollLeft();
      const layoutInfo = editor.getLayoutInfo();
      domNode.style.position = 'absolute';
      domNode.style.top  = (top - scrollTop) + 'px';
    ...
    // Reposition on scroll and layout changes
    const scrollDispose  = editor.onDidScrollChange(positionWidget);
    const layoutDispose  = editor.onDidLayoutChange(positionWidget);
    widget._dispose = () => { scrollDispose.dispose(); layoutDispose.dispose(); };

    merge[key].push(widget);
  });
}
